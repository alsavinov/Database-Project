...
IF EXISTS (SELECT 1 FROM pg_database WHERE datname = dbname) THEN
   ...
   RETURN 'Database has already existed'::text;
ELSE
   ...
   -- для доп.информации можно добавить hostaddr=127.0.0.1 port=5432 dbname=mydb user=postgres password=mypasswd
	...
	/*PERFORM dblink_exec('port=5432 dbname=tmp user=postgres password=***',
                     'CREATE DATABASE ' || quote_ident(dbname));*/
					 
	PERFORM dblink_exec('port=5432 dbname=tmp user=postgres password=111',
                     'CREATE DATABASE ' || quote_ident(dbname));
	PERFORM dblink_exec('port=5432 dbname=tmp user=postgres password=111',
                     'DO $$
				      BEGIN
						IF EXISTS (SELECT 1 FROM pg_user WHERE usename = ''admin1'') THEN
							RAISE NOTICE ''User already exists'';
							GRANT ALL PRIVILEGES ON DATABASE ' || quote_ident(dbname) || ' TO admin1;
					  	ELSE
							CREATE USER admin1 WITH PASSWORD ''111'';
							GRANT ALL PRIVILEGES ON DATABASE ' || quote_ident(dbname) || ' TO admin1;
						END IF;
					  END $$;');
					 
	--CREATE USER admin1 WITH PASSWORD '111');
	--execute 'GRANT ALL PRIVILEGES ON DATABASE ' || quote_ident(dbname) || ' TO admin1;';
	--GRANT ALL PRIVILEGES ON DATABASE quote_ident(dbname) TO admin1;
	
	PERFORM dblink_exec('port=5432 dbname=' || quote_ident(dbname) || ' user=admin1 password=111',
                     'create table users(
...
(''Man City''), (''Arsenal''), (''Man United''), (''Chelsea''), (''Brighton''),
(''Real Madrid''), (''Barcelona''), (''Krasnodar''), (''Juventus''), (''Roma''),
(''Monaco''), (''Liverpool''), (''Sevilla''), (''West Ham''), (''Lille'');
...
(''2022-03-21'', 2, 1, ''X'', 2, 2),
(''2022-02-12'', 1, 5, ''1'', 3, 2),
(''2022-01-11'', 2, 3, ''X'', 1, 1),
(''2022-01-04'', 4, 3, ''2'', 0, 1),
(''2022-03-15'', 5, 4, ''1'', 1, 0);
...
CREATE OR REPLACE FUNCTION show_table_players()
	RETURNS TABLE(player_id int, player_name varchar(50), player_position varchar(2), 
					   team_name varchar(50)) AS $$
		BEGIN
			RETURN QUERY
			SELECT players.player_id, players.player_name, players.player_position, 
...
');
	RETURN 'Database was created'::text;	
END IF;
...
IF EXISTS (SELECT 1 FROM pg_database WHERE datname = dbname) THEN
   ...
   --RAISE NOTICE 'Database already exists'; 
	...
	RETURN 'Database was deleted'::text;	
ELSE
	RETURN 'No database with such name'::text;
END IF;
...
CREATE OR REPLACE FUNCTION f_connect_db(dbname text)
  RETURNS text AS
$func$
BEGIN
-- проверка на существование БД с таким же именем
